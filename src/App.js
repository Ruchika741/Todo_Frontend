...
class App extends React.Component {
  ...
  fetchTasks() {
    ...
    fetch("http://127.0.0.1:8000/tasks/task-list/")
  ...
  handleSubmit(e) {
    ...
    let url = 'http://127.0.0.1:8000/tasks/task-create/'

    if (this.state.editing === true) {
      url = `http://127.0.0.1:8000/tasks/task-update/${this.state.activeItem.id}/`
      this.setState({
        editing: false
      })
    }
  ...
  deleteItem(task) {
    ...
    fetch(`http://127.0.0.1:8000/tasks/task-delete/${task.id}/`, {
      method: 'DELETE',
      headers: {
        'Content-type': 'application/json',
        'X-CSRFToken': csrftoken,
  ...
  strikeUnstrike(task) {
    ...
    var url = `http://127.0.0.1:8000/tasks/task-update/${task.id}/`
